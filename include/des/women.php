...
<select onchange="self.location.href = '?' + options[selectedIndex].value;" class="selectpicker ml-auto" name="sorting" data-width="200" data-style="bs-select-form-control" data-title="Default sorting">
  ...
  <?php
   ...
   $do = $_SESSION['do'];
   ?>
  <option value="sort=pop&page=<?=$pa?>&do=<?=$do?>">Popularity</option>
  <option value="sort=lth&page=<?=$pa?>&do=<?=$do?>">Price: Low to High</option>
  <option value="sort=htl&page=<?=$pa?>&do=<?=$do?>">Price: High to Low</option>
</select>
</form>
...
$sql = "SELECT * FROM `product` WHERE `detail` = '$idlist' LIMIT 4";
 if( empty($_GET['page'])&& !empty($_GET['sort'])&& !empty($_GET['do'])){
     if ($_GET['sort'] == "pop") {
...
$sql ="SELECT * FROM product WHERE detail = $idlist $wheres LIMIT 4 ";
}
if (!empty($_GET['page']) && empty($_GET['sort']) && !empty($_GET['do'])) {
    $page = $_GET['page'];
    $n = ($page-1)*4;
    $idlist = $_GET['do'];
    $sql= "SELECT * FROM product WHERE detail = $idlist LIMIT 4 OFFSET $n";
}
 if(!empty($_GET['sort']) && !empty($_GET['page']) && !empty($_GET['do'])){
    $page = $_GET['page'];
    $n = ($page-1)*4;
     if ($_GET['sort'] == "pop") {
    $wheres = "order by rate desc"; 
   ...
   $sql ="SELECT * FROM product WHERE detail = $idlist $wheres LIMIT 4 OFFSET $n";
}
...
<?php
$sort = $_SESSION['sort'];
$sqlp = "SELECT count(*)/4 as counter FROM product WHERE detail = $idlist";
...
for ($i=0; $i <$count ; $i++) { 
?>
<li class="page-item active"><a class="page-link" href="?page=<?= $i+1;?>&sort=<?=$sort;?>&do=<?=$do?>"><?php echo $i + 1 ?></a></li>
<?php
